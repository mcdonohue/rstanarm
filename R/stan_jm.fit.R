...
stan_jm.fit <- function(formulaLong = NULL, dataLong = NULL, formulaEvent = NULL, 
                        ...
                        prior_sigma_lt = cauchy(0, 5, autoscale=TRUE),
  ...
  # Determine whether a latent time model
  is_ltjmm <- !is.null(lt_var)
  ...
  # Offset
  Y_offset <- fetch(y_mod, "offset", pad_length = 20)
  standata$has_offset <- has_offset <-
    fetch_array(y_mod, "has_offset", pad_length = 20)
  for(i in 1:20){
    standata[[paste0('y', i, '_offset')]] <- if (has_offset[i]) Y_offset[[i]] else 
      as.array(integer(0))  
  }
  ...
  if (length(cnms) > 1L) {
    ...
    standata$bN2 <- 0L
    standata$bK2 <- 0L
    standata$bK2_len <- as.array(rep(0,20L))
    standata$bK2_idx <- get_idx_array(rep(0,20L))
    for(i in 1:20){
  ...
  if (is_jm) { # begin jm block
    if (is_ltjmm) stop('Joint model not yet supported for ltjmm.')
...
#    if (is_ltjmm) user_prior_sigma_lt = y_user_prior_sigma_lt_stuff,
  ...
  if (M  > 1L) 
    cat("Fitting a multivariate", if (is_jm) "joint" else if(is_ltjmm) "ltjmm" else "mvmer", "model.\n\n")
